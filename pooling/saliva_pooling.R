...
library(lmtest)
library(sandwich)
pooling <- read.csv('./pooling_n1.csv')
...
pooling.c.high <- pooling.c2[which(pooling.c2$ct1>=30),]
...
pooling2 <- read.csv('./pooling2.csv')
...
p2colors <- data.frame(ratio=levels(pooling2.c$ratio),color=c('darkred','red1','lightpink2'))
...
plot(pooling2.c$ct1,pooling2.c$ctp,col=alpha(p2colors$color[match(pooling2.c$ratio,p2colors$ratio)],0.8),
...
plot(pooling.c$ct1,pooling.c$ctp,col=alpha(pcolors$color[match(pooling.c$ratio,pcolors$ratio)],0.8),
     pch=16,xlab = 'Starting CT',ylab='Pooling CT',cex=1.25,
...
p3 <- rbind(pooling.c[,-c(4,6)],pooling2.c)
...
impact <- read.csv('./IMPACT_saliva.csv')
...
#names(impact.c) <- c('Sample.ID','1:1','1:5','1:10','1:20')
names(impact.c) <- c('Sample.ID','1','5','10','20')
impact.m <- melt(impact.c,id.vars = c('Sample.ID','1'))
names(impact.m) <- c('Sample.ID','ct1','ratio','ct')

#plot(impact.m$condition,impact.m$ct,type='p')
library(ggplot2)
ggplot(data=impact.m, aes(x=condition,y=ct))+
  geom_dotplot(binaxis='y',stackdir='center',aes(fill=condition),alpha=0.5,stackratio = 0.75,dotsize=1,binwidth = 0.5)+
...
mod.me.d <- as.data.frame(t(apply(ranef(mod.me)$sample,
                                  1,function(x) fixef(mod.me) + x)))
...
pred_me <- melt(apply(mod.me.d,1,function(x) x[1] + x[2]*0:3),
                value.name = "sample")
#some re-formatting for the plot
names(pred_me)[1:2] <- c("condition","sample")
pred_me$condition[which(pred_me$condition==1)] <- 5
pred_me$condition[which(pred_me$condition==2)] <- 10
pred_me$condition[which(pred_me$condition==3)] <-20
pred_me$sample <- as.factor(pred_me$sample)

#plot with actual data
ggplot(pred_me,aes(x=condition,y=value,color=sample))+
  geom_line()+
  geom_point(data=pooling2.c,aes(x=ratio,y=dct))+
